...
struct ImGuiSettingsHandler
{
    ...
    void        (*ReadInitFn)(ImGuiContext* ctx, ImGuiSettingsHandler* handler);                                // comms::read: Called before reading (in registration order)
    void*       (*ReadOpenFn)(ImGuiContext* ctx, ImGuiSettingsHandler* handler, const char* name);              // comms::read: Called when entering into a new ini entry e.g. "[Window][Name]"
    void        (*ReadLineFn)(ImGuiContext* ctx, ImGuiSettingsHandler* handler, void* entry, const char* line); // comms::read: Called for every line of text within an ini entry
    void        (*ApplyAllFn)(ImGuiContext* ctx, ImGuiSettingsHandler* handler);                                // comms::read: Called after reading (in registration order)
    void        (*WriteAllFn)(ImGuiContext* ctx, ImGuiSettingsHandler* handler, ImGuiTextBuffer* out_buf);      // comms::write: Output every entries into 'out_buf'
    void*       UserData;
...
struct ImGuiContext
{
    ...
    ImGuiLastItemData       LastItemData;                       // Storage for last submitted item (run_collection by ItemAdd)
...
// (This doesn't need a constructor because we zero-clear it as part of ImGuiWindow and all frame-temporary data are run_collection on Begin)
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImRect                  OuterRectClipped;                   // == Window->Rect() just after run_collection in Begin(). == window->Rect() for root window.
...
//#endif
    ...
    IMGUI_API void          BeginColumns(const char* str_id, int count, ImGuiOldColumnFlags flags = 0); // run_collection number of columns. use an identifier to distinguish multiple column sets. close with EndColumns().
